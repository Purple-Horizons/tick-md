  ...
  request(request, resultSchema, options2) {
    return new Promise((resolve, reject) => {
      ...
      (_c = options2 === null || options2 === void 0 ? void 0 : options2.signal) === null || _c === void 0 ? void 0 : _c.addEventListener("abort", () => {
        var _a2, _b2;
        const reason = (_a2 = options2 === null || options2 === void 0 ? void 0 : options2.signal) === null || _a2 === void 0 ? void 0 : _a2.reason;
...
// ../packages/tick-core/dist/parser/parse.js
var import_gray_matter = __toESM(require_gray_matter(), 1);
var import_yaml = __toESM(require_dist(), 1);
function parseTickFile(content) {
  const result = parseTickFileWithErrors(content);
  return result;
}
function parseTickFileWithErrors(content) {
  const parseErrors = [];
  let frontmatter = {};
  let body = content;
  try {
  ...
  const meta = {
    ...
    created: safeString(frontmatter.created, (/* @__PURE__ */ new Date()).toISOString()),
    updated: safeString(frontmatter.updated, (/* @__PURE__ */ new Date()).toISOString()),
...
function parseTaskBlocksWithErrors(content) {
  const tasks = [];
  const errors = [];
  const seenIds = /* @__PURE__ */ new Set();
  const { tasks: structuredTasks, errors: structuredErrors } = parseStructuredTaskBlocksWithErrors(content);
...
function parseStructuredTaskBlocksWithErrors(content) {
  const tasks = [];
  const errors = [];
  ...
  for (let i = 0; i < matches.length; i++) {
    ...
    let metadata;
    try {
      metadata = import_yaml.default.parse(yamlContent);
      ...
      minimalTask.parse_error = true;
      minimalTask.raw_yaml = yamlContent;
    ...
    const task = {
      ...
      created_at: safeString(metadata.created_at, (/* @__PURE__ */ new Date()).toISOString()),
      updated_at: safeString(metadata.updated_at, (/* @__PURE__ */ new Date()).toISOString()),
...
function createMinimalTask(taskId, title) {
...
function validatePriority(priority) {
  ...
  if (typeof priority === "string") {
    ...
    if (text.includes("urgent") || text.includes("critical") || text.includes("p0"))
      return "urgent";
    if (text.includes("high") || text.includes("p1"))
      return "high";
    if (text.includes("medium") || text.includes("normal") || text.includes("p2"))
      return "medium";
    if (text.includes("low") || text.includes("p3"))
      return "low";
  }
  return "medium";
}
function parseFreeformTaskBlocks(content, excludeIds) {
  const tasks = [];
  const freeformHeaderRegex = /^##\s+([A-Z]+-\d+)\s*[:·\-–—]\s*(.+?)$/gm;
  const matches = [...content.matchAll(freeformHeaderRegex)];
  for (let i = 0; i < matches.length; i++) {
    const match = matches[i];
    const taskId = match[1];
    if (excludeIds.has(taskId))
      continue;
    ...
    const task = {
      ...
      updated_at: metadata.updated_at || (/* @__PURE__ */ new Date()).toISOString(),
      due_date: metadata.due_date,
      tags: metadata.tags || [],
...
function parseFreeformMetadata(taskBlock) {
  const metadata = {};
  const statusMatch = taskBlock.match(/\*\*Status:\*\*\s*(.+?)(?:\n|$)/i);
  if (statusMatch) {
    metadata.status = normalizeStatus(statusMatch[1].trim());
  }
  const priorityMatch = taskBlock.match(/\*\*Priority:\*\*\s*(.+?)(?:\n|$)/i);
  if (priorityMatch) {
    metadata.priority = normalizePriority(priorityMatch[1].trim());
  }
  const ownerMatch = taskBlock.match(/\*\*(?:Owner|Assigned(?:\s*to)?|Assignee):\*\*\s*(.+?)(?:\n|$)/i);
  if (ownerMatch) {
  ...
  const dependsMatch = taskBlock.match(/\*\*(?:Depends\s*on|Dependencies|Blocked\s*by):\*\*\s*(.+?)(?:\n|$)/i);
  if (dependsMatch) {
    metadata.depends_on = parseTaskList(dependsMatch[1]);
  }
  const blocksMatch = taskBlock.match(/\*\*Blocks:\*\*\s*(.+?)(?:\n|$)/i);
  if (blocksMatch) {
    metadata.blocks = parseTaskList(blocksMatch[1]);
  }
  const tagsMatch = taskBlock.match(/\*\*Tags:\*\*\s*(.+?)(?:\n|$)/i);
  if (tagsMatch) {
    metadata.tags = tagsMatch[1].split(/[,;]/).map((t) => t.trim()).filter((t) => t);
  }
  const dueMatch = taskBlock.match(/\*\*(?:Due(?:\s*date)?|Deadline):\*\*\s*(.+?)(?:\n|$)/i);
  if (dueMatch) {
    metadata.due_date = dueMatch[1].trim();
  }
  const createdByMatch = taskBlock.match(/\*\*Created\s*by:\*\*\s*(.+?)(?:\n|$)/i);
  if (createdByMatch) {
...
function parseTaskList(text) {
  return text.split(/[,;\s]+/).map((t) => t.trim()).filter((t) => /^[A-Z]+-\d+$/.test(t));
}
function extractFreeformDescription(taskBlock) {
...
// ../packages/tick-core/dist/parser/serialize.js
var import_yaml2 = __toESM(require_dist(), 1);
function serializeTickFile(tickFile) {
  const parts = [];
  parts.push("---");
  ...
  if (tickFile.tasks.length > 0) {
    ...
    for (const task of tickFile.tasks) {
      parts.push(`### ${task.id} \xB7 ${task.title}`);
      parts.push("");
      const metadata = {
        id: task.id,
        status: task.status,
      ...
      if (task.deliverables && task.deliverables.length > 0) {
        metadata.deliverables = task.deliverables.map((d) => {
          const entry = {
            name: d.name,
            type: d.type
          };
      ...
      parts.push("```yaml");
      parts.push(import_yaml2.default.stringify(metadata).trim());
      parts.push("```");
      parts.push("");
      if (task.description) {
...
// ../packages/tick-core/dist/validation/validator.js
function validateTickFile(tickFile) {
  const errors = [];
  const warnings = [];
...
function validateTask(task, tickFile, errors, warnings) {
  if (!task.id) {
    errors.push({
      type: "error",
...
function findCircularDependencies(tasks) {
  ...
  function dfs(taskId, path2) {
    if (recursionStack.has(taskId)) {
      const cycleStart = path2.indexOf(taskId);
      if (cycleStart >= 0) {
        cycles.push([...path2.slice(cycleStart), taskId]);
      }
      return;
    }
...
// ../packages/tick-core/dist/io/tick-file.js
var import_node_fs = __toESM(require("node:fs"), 1);
var import_node_path = __toESM(require("node:path"), 1);
var TICK_FILENAME = "TICK.md";
function findTickFile(baseDir = process.cwd()) {
  const tickPath = import_node_path.default.join(baseDir, TICK_FILENAME);
  return import_node_fs.default.existsSync(tickPath) ? tickPath : null;
}
function readTickFileStateSync(tickPath) {
...
function assertFileUnchanged(filePath, expected) {
  if (!expected)
    return;
  const current = import_node_fs.default.statSync(filePath);
...
function writeTickFileAtomicSync(tickFile, tickPath, expected) {
  const filePath = tickPath || findTickFile();
  if (!filePath) {
    throw new Error("TICK.md not found. Run 'tick init' to create a project.");
  }
  ...
  const tempPath = import_node_path.default.join(dir, `${import_node_path.default.basename(filePath)}.${process.pid}.${Date.now()}.tmp`);
  import_node_fs.default.writeFileSync(tempPath, content, "utf-8");
...
function saveTickFile(tickPath, tickFile, readState) {
  tickFile.meta.updated = (/* @__PURE__ */ new Date()).toISOString();
...
function addTask(tickFile, params) {
...
function getTaskOrThrow(tickFile, taskId) {
...
function claimTask(tickFile, taskId, agent) {
  ...
  if (task.claimed_by)
    throw new Error(`Task ${taskId} is already claimed by ${task.claimed_by}`);
  if (task.status === "done")
...
function releaseTask(tickFile, taskId, agent) {
  const task = getTaskOrThrow(tickFile, taskId);
...
function registerAgent(tickFile, params) {
...
function reopenTask(tickFile, params) {
...
function deleteTask(tickFile, taskId, force = false) {
  const task = getTaskOrThrow(tickFile, taskId);
  if (!force) {
...
function editTask(tickFile, params) {
  const task = getTaskOrThrow(tickFile, params.taskId);
  const now = nowIso();
  ...
  if (params.assignedTo !== void 0)
    task.assigned_to = params.assignedTo || null;
  if (params.tags)
    task.tags = params.tags;
  if (params.dependsOn)
    task.depends_on = params.dependsOn;
  if (params.note)
    task.history.push({ ts: now, who: params.agent, action: "commented", note: params.note });
  task.updated_at = now;
}
function statusPayload(tickFile) {
  ...
  try {
    switch (name) {
      ...
      case "tick_comment": {
        ...
        if (!taskId || !agent || !note)
          throw new Error("taskId, agent, and note are required");
        const { tickPath, tickFile, readState } = loadTickFile();
      ...
      case "tick_agent_register": {
        ...
        const registered = registerAgent(tickFile, {
          name: nameValue,
          type: asString(args.type) || "human",
          roles: toArrayStrings(args.roles),
          status: asString(args.status) || "idle"
        });
        saveTickFile(tickPath, tickFile, readState);
        return {
          content: [
      ...
      case "tick_undo": {
        ...
        const log = (0, import_node_child_process.execSync)("git log -1 --pretty=%B", { encoding: "utf-8" }).trim();
        if (!force && !log.includes("[tick]")) {
          throw new Error("Last commit is not a tick commit. Use force=true to override.");
        }
        (0, import_node_child_process.execSync)("git revert --no-edit HEAD", { stdio: "pipe" });
